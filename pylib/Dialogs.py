...
class DualListDialog( QDialog ):
    ...
    def __init__( self, title, owned_items, action_data, fields, parent ):
        ...
        fill_avail = action_data['fill_avail']
        slots = action_data['slots']
        self.slots_name = slots_name = action_data['slots_name']
        category_field = action_data['category_field']
        self.tool_tip = tool_tip = action_data['tool_tip']
        self.add = add = action_data['add']
        self.remove = remove = action_data['remove']
    ...
    def add_pressed( self ):
        ...
        if type( slots_new_value ).__name__ == 'str':
            self.slots_label.setText( '<b>{}:</b>{}'.format( self.slots_name, slots_new_value ) )
        if remove is True:
            current_list.takeItem( current_list.currentRow() )
        if type( new_display ).__name__ != 'str':
            new_display = current_item.text()
        add_item_to_listbox( self.chosen_list, current_item.data( QtCore.Qt.UserRole ), self.tool_tip, self.fields, current_list )
    ...
    def remove_pressed( self ):
        ...
        remove_return = self.remove( current_item.data( QtCore.Qt.UserRole ), self.fields )
        ...
        if replace is True:
            ...
            add_item_to_listbox( original_list, current_item.data( QtCore.Qt.UserRole ), self.tool_tip, self.fields, original_list )
